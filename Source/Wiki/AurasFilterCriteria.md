...
| Blizzard | Debuffs **you applied** that Blizzard itself would also show on its own default nameplates (self-applied only — see note) | `ShowBlizzardForEnemy` | two peer `AddAuraGroup`s: `IMPORTANT\|PLAYER` + `candidateFilters.nameplateShowPersonal = true`, and `!IMPORTANT\|PLAYER` + the same `candidateFilters` | **Matches Plater-Nameplates' `DB_AURA_SHOW_AS_BLIZZARD` exactly (2026-08-17)** — confirmed by reading `Plater_Auras.lua`. Blizzard curates default-nameplate debuffs via two independent flags: `nameplateShowAll` (shown for anyone) and `nameplateShowPersonal` (shown only when self-applied); the `IMPORTANT`/`!IMPORTANT` split (not `nameplateShowAll`/`nameplateShowPersonal` directly) is how Plater gets full coverage while keeping every group `PLAYER`-restricted. **Known tradeoff, reintroduced on purpose**: two concurrently-active groups for this one toggle means `SortOrder` (e.g. TimeLeft) is not globally correct across them when "Blizzard" is active alone — Blizzard's `AuraContainer` never merges sort order across `AddAuraGroup`s, only within each one (see `AurasWidgetImplementation.md` §6). A same-day single-group fix (`nameplateShowPersonal` only, no `IMPORTANT` split) avoided this cost but dropped Blizzard-flagged-but-not-`nameplateShowPersonal` coverage; reverted in favor of matching Plater. `PLAYER` is hardcoded by deliberate design (2026-08-16) — "Blizzard" always means "mine, and Blizzard-flagged", not "anyone's" |
