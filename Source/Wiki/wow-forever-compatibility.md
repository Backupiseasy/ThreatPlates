...
| 29 | `Constants.lua` (friendly unit type CVars), `CVarsManager.lua`, `Options.lua`, `Nameplate.lua`, `Commands.lua` | The friendly NPC/minion/pet/guardian/totem settings had no effect: Midnight renamed the CVars (`nameplateShowFriendlyPlayerMinions/-Pets/-Guardians/-Totems`, `nameplateShowFriendlyNpcs`) and Forever uses the new names; the old ones return `nil` there (`nameplateShowFriendlyNPCs`/`...Npcs` resolve to the same CVar, names are case-insensitive). Blizzard's own settings dialogs (`Blizzard_SettingsDefinitions_Frame/Nameplates.lua`) in the Retail, Classic Era, TBC Anniversary and Mists UI sources use the new names as well (only the outdated `classic_beta` source mirror still has the old ones), i.e. no supported client uses the old names any more - this is also the cause of the TBC report "options for pets/minions/... cannot be checked" (comment #8597) | Fixed new names in `Constants.lua`; old names (and the detection tried in between) removed from `Constants.lua`, `CVarsManager.lua` (`COMBAT_PROTECTED`), `Options.lua`, `Nameplate.lua`. `/tptp debug MidnightAPI` now ends with a CVar check (does every CVar used for the friendly unit types exist?). **Verified in-game on Forever** (see "CVar check" below); the other Classic clients are only verified via the UI sources. |
...
| 34 | `Options.lua` (`CreateUnitGroupsVisibility`, Visibility tab) | The "Pets" option under "Show Friendly Units" could be enabled while "Players" was disabled, but friendly pets get no nameplates then (`nameplateShowFriendlyPlayers` off hides them; Blizzard's own dialog makes "Minions" a child of "Friendly Players" via `SetParentInitializer`, but has no checkboxes for pets/guardians/totems) | "Pets" (Friendly) is `disabled` while `nameplateShowFriendlyPlayers` is off (`IsFriendlyPlayerVisibilityDisabled`). "All Minions" stays enabled on purpose (it is a convenience setter for all minion types). **Confirmed in-game** for pets; guardians and totems are not verified yet and stay enabled. |
...
| Friendly pets get no nameplate while friendly players are disabled (fix #34) | ✅ confirmed |
| Friendly guardians / totems get no nameplate while friendly players are disabled (if so: also disable their options, see fix #34) | ⏳ open |
