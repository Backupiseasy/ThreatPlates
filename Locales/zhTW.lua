...
L["|cffFFFFFFMigrate Custom Nameplates Settings|r"] = "|cffFFFFFF轉移自訂血條設定|r"
...
L["A custom nameplate with these triggers already exists: %s. You cannot use two custom nameplates with th same trigger. The current custom nameplate was therefore disabled."] = "已有自訂血條使用這些觸發: %s。因為不能有兩個自訂血條使用相同的觸發，目前的自訂血條已被停用。"
...
L["A custom nameplate with this trigger already exists: %s. You cannot use two custom nameplates with the same trigger."] = "已有自訂血條使用這個觸發條件: %s。同一個觸發條件無法使用兩個自訂血條。"
L["A custom nameplate with this trigger is already defined: %s. You cannot define two custom nameplates with the same trigger."] = "使用這個觸發條件的自訂血條已經設定好了: %s。無法設定兩個兩個自訂血條使用相同的觸發條件。"
L["A custom nameplate with this trigger is already defined: %s. You cannot use two custom nameplates with the same trigger."] = "使用這個觸發條件的自訂血條已經設定好了: %s。同一個觸發條件無法使用兩個自訂血條。"
L["A to Z"] = "A 到 Z"
L["About"] = "關於"
...
L["Find a suitable icon base on aura/spell ID or aura/spell name. This does not work for name-based custom nameplates."] = "依據光環/法術 ID 或光環/法術名稱尋找適合的圖示。不適用於以單位名字為主的自訂血條。"
...
L["Import a profile from a import string from another user."] = "從另一位使用者的匯入字串匯入設定檔。"
L["Import a profile from another player from an import string."] = "從另一位玩家的匯入字串匯入設定檔。"
L["Import and export profiles that can be shared with other players."] = "匯入和匯出可以和其他玩家分享的設定檔。"
...
L["Shows a glow effect on the healthbar of this custom nameplate."] = "在這個自訂血條的血量條上顯示發光效果。"
...
L["Social"] = "社交"
L["Something went wrong with importing the profile. Please check the import string."] = "匯入設定檔時出了點問題，請檢查匯入字串。"
L["Something went wrong with importing your profile. Please check the import string."] = "匯入你的設定檔時出了點問題，請檢查匯入字串。"
L["Sort A-Z"] = "排序 A-Z"
...
L["The import string contains invalid custom nameplate settings and cannot be imported. Verify that the import string was generated from the same Threat Plates version that you are using."] = "匯入字串包含無效的自訂血條設定，無法被匯入。請確認匯入字串是從和你使用相同的威力血條版本所產生的。"
...
L["The import string has an invalied format and cannot be imported. Verify that the import string was generated from the same Threat Plates version that you are using currently."] = "匯入字串包含無效的格式，無法匯入。請確認匯入字串是由相同版本的威力血條所產生的。"
...
L["This option is disabled as you are still using legacy custom nameplates. Migrate your custom nameplates to the new version (using the Migration button at the top) to enable this option."] = "這個選項是停用的，因為目前仍在使用傳統的自訂血條設定。要啟用這個選項，請先 (使用最上方的轉移按鈕) 將自訂血條轉移到新版本 。"
L["This option is disabled as you are still using the obsolete custom nameplates format. Migrate your custom nameplates to the new format (using the Migration button at the top) to enable this option."] = "這個選項是停用的，因為目前仍在使用過時的自訂血條格式。要啟用這個選項，請先 (使用最上方的轉移按鈕) 將自訂血條轉移成新格式 。"
...
L["You need to convert your custom nameplates to the current format before you can add legacy custom nameplates."] = "新增傳統自訂血條前，必須先將自訂血條轉換成新格式。"
